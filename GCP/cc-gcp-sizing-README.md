...
## Usage
For best results, log in to the GCP Console for the organization account with the admin/owner credentials. If an orginization account is not in use, log in to each standalone project account with admin/owner credentials.
   
### Prerequisites (if not using GCP CloudShell)  
* Install and configure the GCP CLI 
* Install `jq` 
    
### Execution  
1. Launch the GCP CloudShell from the top menu bar.  
3. Upload the sizing script to your CloudShell instance.  
4. `chmod +x ./cc-workload-sizing-gcp.sh` to update permissions on the sizing script.  
5. Execute the script `./cc-workload-sizing-gcp.sh [-r]  
   ...
   * Optional flags are available:  
      * `-h` display help info  
      * `-o` Specify a single Organization ID to scan. If not provided, the script will attempt to list and scan all organizations you have access to."
      * `-l <region>` specify single location to scan  
